...
class LogTest extends JUnitSuite {
  ...
  def testSizeBasedLogRoll() {
    ...
    // create a log
    val log = new Log(logDir, LogConfig(segmentSize = segmentSize), needsRecovery = false, time.scheduler, time = time)
  ...
  def testAppendAndReadWithSequentialOffsets() {
    val log = new Log(logDir, LogConfig(segmentSize = 71), needsRecovery = false, time.scheduler, time = time)
  ...
  def testReadAtLogGap() {
    val log = new Log(logDir, LogConfig(segmentSize = 300), needsRecovery = false, time.scheduler, time = time)
  ...
  def testReadOutOfRange() {
    ...
    val log = new Log(logDir, LogConfig(segmentSize = 1024), needsRecovery = false, time.scheduler, time = time)
  ...
  def testLogRolls() {
    ...
    val log = new Log(logDir, LogConfig(segmentSize = 100), needsRecovery = false, time.scheduler, time = time)
    val numMessages = 100
  ...
  def testCompressedMessages() {
    ...
    val log = new Log(logDir, LogConfig(segmentSize = 10), needsRecovery = false, time.scheduler, time = time)
  ...
  def testMessageSizeCheck() {
    ...
    val log = new Log(logDir, LogConfig(maxMessageSize = maxMessageSize), needsRecovery = false, time.scheduler, time = time)
  ...
  def testLogRecoversToCorrectOffset() {
    ...
    val config = LogConfig(segmentSize = segmentSize, indexInterval = indexInterval, maxIndexSize = 4096)
  ...
  def testIndexRebuild() {
    ...
    val numMessages = 200
    val config = LogConfig(segmentSize = 200, indexInterval = 1)
  ...
  def testTruncateTo() {
    ...
    // create a log
    val log = new Log(logDir, LogConfig(segmentSize = segmentSize), needsRecovery = false, scheduler = time.scheduler, time = time)
    assertEquals("There should be exactly 1 segment.", 1, log.numberOfSegments)
  ...
  def testIndexResizingAtTruncation() {
    ...
    val msgPerSeg = 10
    val segmentSize = msgPerSeg * (setSize - 1) // each segment will be 10 messages
    val config = LogConfig(segmentSize = segmentSize)
  ...
  def testBogusIndexSegmentsAreRemoved() {
    ...
    val set = TestUtils.singleMessageSet("test".getBytes())
    val log = new Log(logDir, 
                      LogConfig(segmentSize = set.sizeInBytes * 5, 
                                maxIndexSize = 1000, 
                                indexInterval = 1),
                      needsRecovery = false,
                      time.scheduler,
  ...
  def testReopenThenTruncate() {
    val set = TestUtils.singleMessageSet("test".getBytes())
    val config = LogConfig(segmentSize = set.sizeInBytes * 5, 
                           maxIndexSize = 1000, 
                           indexInterval = 10000)
  ...
  def testAsyncDelete() {
    ...
    val config = LogConfig(segmentSize = set.sizeInBytes * 5, fileDeleteDelayMs = asyncDeleteMs, maxIndexSize = 1000, indexInterval = 10000)
    val log = new Log(logDir,
                      config,
                      needsRecovery = true,                      
                      time.scheduler,
  ...
  def testOpenDeletesObsoleteFiles() {
    val set = TestUtils.singleMessageSet("test".getBytes())
    val config = LogConfig(segmentSize = set.sizeInBytes * 5, maxIndexSize = 1000)
    var log = new Log(logDir,
                      config,
                      needsRecovery = false,
                      time.scheduler,
