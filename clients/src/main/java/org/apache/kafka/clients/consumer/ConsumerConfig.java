...
public class ConsumerConfig extends AbstractConfig {
    ...
    private static final String GROUP_ID_DOC = "A unique string that identifies the consumer group this consumer belongs to. Partitions will be divided up within each group so that each partition has a consumer in each consumer group. This property is required if the consumer uses either the group management functionality by using <code>subscribe(topic)</code> or the Kafka-based offset management strategy.";
    ...
    private static final String AUTO_OFFSET_RESET_DOC = "What to do when there is no initial offset in Kafka or if the current offset is smaller than the oldest offset the server maintains: <ul><li>smallest: automatically reset the offset to the smallest offset<li>largest: automatically reset the offset to the largest offset</li><li>disable: throw exception to the consumer if no previous offset is found for the consumer's group</li><li>anything else: throw exception to the consumer.</li></ul>";
    ...
    private static final String MAX_PARTITION_FETCH_BYTES_DOC = "The amount of data per-partition the client will request. The total memory used for a request will be <code>#partitions * max.partition.fetch.bytes</code>. This size must be at least as large as the maximum message size the server allows or else it is possible for the producer to send messages larger than the consumer can fetch. If that happens, the consumer can get stuck trying to fetch a large message on a certain partition.";
